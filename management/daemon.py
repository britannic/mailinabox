...
def add_security_headers(response):
	if response.mimetype == "text/html":
		# The Munin panel proxies HTML generated by the munin package (served from
		# /var/cache/munin/www via the /munin/ routes), not our Jinja templates, so
		# those pages cannot carry a per-request nonce and would break under the
		# strict script-src policy. Exempt them: they are admin-authenticated,
		# locally generated, static monitoring pages, and nginx's frame-ancestors
		# CSP still applies. Every other (panel) page keeps the strict CSP.
		if not request.path.startswith("/munin/"):
